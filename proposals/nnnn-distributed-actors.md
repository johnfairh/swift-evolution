...
To summrize this section with an example, let us have a look at the following `Greeter` definition, and the `test` function accepting a `greeter` reference:
...
// Invocations:
// remote greeter:       try await greeter.greet(name: "Alice")
// internally, invokes: (try await $distribute_greet(name: "Alice"))
// 
// local greeter:       try await greeter.greet(name: "Alice")
// internally invokes: (greet(name: "Alice"))
```
